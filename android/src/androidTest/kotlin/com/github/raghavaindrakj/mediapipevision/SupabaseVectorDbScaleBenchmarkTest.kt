 ...
 * using real Gemini-extracted embeddings from the four mocktail photos.
 *
 * Skipped unless these Gradle project properties are set:
 *   -PsupabaseUrl=... -PsupabaseKey=... -PgeminiApiKey=...
 */
@RunWith(AndroidJUnit4::class)
class SupabaseVectorDbScaleBenchmarkTest {
    ...
    fun recognize_at50Dishes100SamplesEach(): Unit = runBlocking {
        ...
        assumeTrue("Set supabaseUrl, supabaseKey, geminiApiKey via -P",
            supabaseUrl.isNotEmpty() && supabaseKey.isNotEmpty() && geminiApiKey.isNotEmpty())

        val dishCount = 5
        val samplesPerDish = 10

        // Extract four real embeddings via Gemini.
        val vectorizer = GeminiVectorizer.create(geminiApiKey, dimensions = 1536)
        ...
        val database = SupabaseVectorDb.create(supabaseUrl, supabaseKey, EmbedderType.GEMINI)
    ...
    private companion object {
        const val TAG = "SupabaseScaleBenchmark"
    }
}
