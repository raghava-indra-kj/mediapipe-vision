...
import com.github.raghavaindrakj.mediapipevision.infra.vectorizer.gemini.GeminiVectorizer
...
/**
 * Integration test exercising [GeminiVectorizer] with [SupabaseVectorDb] over real HTTP.
 ...
 *
 * Skipped unless these Gradle project properties are set (pass with `-P`):
 *   -PsupabaseUrl=https://your-project.supabase.co
 *   -PsupabaseKey=your-key
 *   -PgeminiApiKey=your-gemini-api-key
 *
 * Example:
 *   .\gradlew.bat :mediapipe-vision:connectedDebugAndroidTest
 *       -PsupabaseUrl=https://gsuzqxeappsthdilffko.supabase.co
 *       -PsupabaseKey=eyJ...
 *       -PgeminiApiKey=AIzaSyD...
 */
@RunWith(AndroidJUnit4::class)
class SupabaseGeminiIntegrationTest {

    private lateinit var vectorizer: GeminiVectorizer
    ...
    fun setUp() {
        ...
        val geminiApiKey = args.getString("geminiApiKey", "")

        assumeTrue("Set supabaseUrl, supabaseKey, geminiApiKey via -P", supabaseUrl.isNotEmpty() && supabaseKey.isNotEmpty() && geminiApiKey.isNotEmpty())

        vectorizer = GeminiVectorizer.create(geminiApiKey, dimensions = 1536)
        database = SupabaseVectorDb.create(supabaseUrl, supabaseKey, EmbedderType.GEMINI)
    }
    ...
    private companion object {
        const val TAG = "SupabaseGeminiIntegration"
    }
}
